...
function Store() {
  ...
  function fecthSufferingsByCategory(cat) {
    API.fecthSufferingsByCategory(cat)
      .then(res => {
  ...
  function fetchStoreProducts(data) {
  ...
  function handleChangeSuffering(suff) {
    this.setState(
      {
  ...
  useEffect(() => {
    fetchCategories();
    fetchSufferingsByCategory(this.state.selectedCategoryId);
    let data = {};
    data.catId = this.state.selectedCategoryId;
    data.suff = this.state.selectedSuffering;
